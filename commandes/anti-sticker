  ...
  if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
    ...
    try {
      ...
      if (msg.message.stickerMessage) {
        ...
        const caption = `Anti-delete-sticker by BONIPHACE-MD\nMessage de @${senderId} a été supprimé.`;
        const imageCaption = { image: { url: './media/deleted-sticker.jpg' }, caption, mentions: [msg.key.participant] };

        // Send a message to the group indicating a sticker was deleted
        await zk.sendMessage(origineMessage.from, { text: `Un sticker a été supprimé par @${senderId}.`, mentions: [msg.key.participant] });
